...
import com.iyy.dingping.ui.personal.MyLetterListView.OnTouchingLetterChangedListener;
...
public class ActivitySelectCity extends BaseActivity{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		personList = (ListView) findViewById(R.id.list_view);//城市list
		allCity_lists = new ArrayList<City>();
		letterListView = (MyLetterListView) findViewById(R.id.MyLetterListView01);//竖向的缩写列表
		lng_city_lay = (LinearLayout) findViewById(R.id.lng_city_lay);//定位的linearlayout
		sh = (EditText) findViewById(R.id.sh);//搜索框中的文本
		lng_city = (TextView) findViewById(R.id.lng_city);//正在定位的文本，也是最终得到的文本
		...
		personList.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				intent.putExtra("lngCityName", ShowCity_lists.get(arg2).name+"市");
				setResult(99,intent);
				finish();
			}
		});
		...
		Thread thread = new Thread(){
			...
			public void run() {
				hotCityInit();
				//当线程中需要进行的事件结束后，可以发送mes进行关闭dialog操作
...
//				personList.setAdapter(adapter);
	...
	public  ProgressDialog showProgress(Activity activity, String hintText) {
